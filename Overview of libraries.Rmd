...
# Now roll up per-library into the four categories you requested:
library_summary <- df_genus_2504_native %>%
  ...
  summarise(
    ...
    # bleed-over is summed across kingdom groups (use total_bleed because we doubled)
...
df_DNA_contribution_2504 <- library_summary %>%
  ...
  mutate(
    ...
    # replace any NA proportions with 0 so stacked bars sum neatly
